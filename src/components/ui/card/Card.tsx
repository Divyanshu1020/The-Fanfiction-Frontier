  ...
  return (
    <div className=" bg-white dark:bg-[#171717] rounded-md shadow-[0_0_0_1px_#1717170d]">
      ...
      <div className=" max-h-[250px] p-3">
        <div className=" flex flex-row">
          <div className=" flex flex-row gap-2 items-center">
            <div className=" p-1 cursor-pointer whitespace-nowrap hover:bg-[#3b49df1a]  rounded-full relative ">
              {title ? (
                <div className="w-7 h-7 flex items-center justify-center bg-slate-200 dark:bg-[#a3a3a3] rounded-full">
                  {author?.name?.slice(0, 1)}
                </div>
              ) : (
                <div className="w-7 h-7 rounded-full  loading-text"></div>
              )}
            </div>
            <div className="  cursor-pointer font-normal flex flex-col gap-1 leading-3">
              {title ? (
                <div className=" font-medium hover:text-[#0217ff]">
                  {author?.name}
                </div>
              ) : (
                <div className=" w-24 h-3 loading-text"></div>
              )}
              {title ? (
                ...
                <div className=" h-3 loading-text"></div>
              )}
            </div>
          </div>
        </div>
        ...
        <div className=" h-full pl-10 cursor-default">
          {title ? (
            ...
            <div className=" flex flex-row items-center justify-between gap-4">
              <div className=" flex flex-row items-center gap-4">
                <div className=" loading-text h-4 w-24"></div>
                <div className=" loading-text h-4 w-24"></div>
              </div>
              <div>
                <div className=" loading-text h-10 w-10"></div>
              </div>
            </div>
          )}
        </div>
